...
void spi_init(void)
{
    ...
    // MGN
    // Pins config in driver_all_board.h
    // SPI1 config in driver_all_board.h
}
...
status_t spi_set_frequency(uint32_t frequency)
{
	uint32_t sourceClock;
    // MGN
    SDCARD_SPI->DIV = SPI_DIV_VAL(Chip_SPI_CalClkRateDivider(SDCARD_SPI, frequency) - 1);
    if(SDCARD_SPI->DIV != 0)
    {
        return kStatus_Success;
    }
...
static SPI_DATA_SETUP_T g_SDCardXferSetup;
...
//	return SPI_MasterTransferBlocking(SPI2, &xfer);	
    ...
    g_SDCardXferSetup.pTx             = in;
    
    if(out != NULL)
        g_SDCardXferSetup.pRx             = out;
    
    g_SDCardXferSetup.Length          = size;
    g_SDCardXferSetup.DataSize        = 8;
    g_SDCardXferSetup.completion_flag = false;
    
    SDCARD_CS_CLR();
    if(out != NULL)
        Chip_SPI_RWFrames_Blocking(SDCARD_SPI, &g_SDCardXferSetup);
    else
        Chip_SPI_WriteFrames_Blocking(SDCARD_SPI, &g_SDCardXferSetup);
    SDCARD_CS_SET();
...
{
    extern volatile uint32_t g_SystemTicks;
...
void sdcard_init(void)
{
	...
	{
        DEBUGOUT("Mount volume failed.\r\n");
	}
		
#if (_FS_RPATH >= 2)
	if (f_chdrive((char const *)&driverNumberBuffer[0]))
	{
		DEBUGOUT("Change drive failed.\r\n");
		return;
	}
#endif	
	FIL fd;
	...
	{
		DEBUGOUT("f_open failed.\r\n");
	}
	else
	{
        DEBUGOUT("readme.txt open success.\r\n");
	}
