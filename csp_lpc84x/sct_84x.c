/*
 * @brief LPC84X State Configurable Timer driver
 *
 * @note
...
/* Initialize SCT (Enable SCT Clock & Reset the SCT)*/
void Chip_SCT_Init(LPC_SCT_T *pSCT)
{
	Chip_Clock_EnablePeriphClock(SYSCON_CLOCK_SCT);
	Chip_SYSCON_PeriphReset(RESET_SCT);
}

/* Shutdown SCT */
void Chip_SCT_DeInit(LPC_SCT_T *pSCT)
{
	Chip_Clock_DisablePeriphClock(SYSCON_CLOCK_SCT);
}
