/*
 * @brief LPC84X Self Wakeup Timer (WKT) chip driver
 *
 * @note
...
{
	if (clkSrc == WKT_CLKSRC_WKTCLKIN) {
//		LPC_IOCON->PIO[0][28] &= ~(3 << 3);	/* Fixed at p0.28 */
		Chip_IOCON_PinSetMode(IOCON_PIO0_28, PIN_MODE_INACTIVE);	
		pWKT->CTRL |= WKT_CTRL_SEL_EXTCLK;	/* using WKTCLKIN */
	}
	else if (clkSrc == WKT_CLKSRC_10KHZ) {
		pWKT->CTRL &= ~WKT_CTRL_SEL_EXTCLK;
...
uint32_t Chip_WKT_GetClockRate(LPC_WKT_T *pWKT)
{
	...
	else if (Chip_WKT_GetClockSource(pWKT) == WKT_CLKSRC_10KHZ) {
		/* Approximately 10KHz */
		return (10 * 1000);
	}
	else {
