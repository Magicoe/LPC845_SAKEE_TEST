...
/** @defgroup SCT_84X CHIP: LPC8xx State Configurable Timer driver
...
#define SCT_CONFIG_NORELOAD_L          	(0x1 << 7)	/*!< prevents the lower match registers from being reloaded */
#define SCT_CONFIG_NORELOAD_H           (0x1 << 8)	/*!< prevents the lower match registers from being reloaded */
#define SCT_CONFIG_NORELOAD_U          	(0x1 << 7)	/*!< prevents the lower match registers from being reloaded (both high & low if UNIFY set) */
#define SCT_CONFIG_INSYNC_P	          	9						/*!< Synchronization for input N (bit 9 = input 0, bit 10 = input 1,..., bit 12 = input 3) */
#define SCT_CONFIG_INSYNC_M          		(0xFFUL << SCT_CONFIG_INSYNC_P)
#define SCT_CONFIG_INSYNC_SET(value)   	(((uint32_t)(value) << SCT_CONFIG_INSYNC_P) & SCT_CONFIG_INSYNC_M)	/*!<  */
#define SCT_CONFIG_AUTOLIMIT_L          	(0x1 << 17)	/*!< match on match register 0 to be treated as a de-facto LIMIT condition */
#define SCT_CONFIG_AUTOLIMIT_H          	(0x1 << 18)	/*!< match on match register 0 to be treated as a de-facto LIMIT condition */
#define SCT_CONFIG_AUTOLIMIT_U          	(0x1 << 17)	/*!< (both high & low if UNIFY set) */
...
#define SCT_EVn_CTRL_MATCHSEL_P       0				/*!< Selects the Match register associated with this event */
#define SCT_EVn_CTRL_MATCHSEL_M       (0xFUL << SCT_EVn_CTRL_MATCHSEL_P)
#define SCT_EVn_CTRL_MATCHSEL_SET(value)	(((uint32_t)(value) << SCT_EVn_CTRL_MATCHSEL_P) & SCT_EVn_CTRL_MATCHSEL_M)
#define SCT_EVn_CTRL_HEVENT						(1 << 4)				/*!< Select L/H counter */
#define SCT_EVn_CTRL_OUTSEL						(1 << 5)				/*!< Selects the outputs/inputs elected by IOSEL */
#define SCT_EVn_CTRL_IOSEL_P					6				/*!< Selects the input or output signal number */
#define SCT_EVn_CTRL_IOSEL_M					(0xFUL << SCT_EVn_CTRL_IOSEL_P)
#define SCT_EVn_CTRL_IOSEL_SET(value)	(((uint32_t)(value) << SCT_EVn_CTRL_IOSEL_P) & SCT_EVn_CTRL_IOSEL_M)
#define SCT_EVn_CTRL_IOCOND_P					10				/*!< Selects the I/O condition for event */
#define SCT_EVn_CTRL_IOCOND_M					(0x3UL << SCT_EVn_CTRL_IOCOND_P)
#define SCT_EVn_CTRL_IOCOND_SET(value)	(((uint32_t)(value) << SCT_EVn_CTRL_IOCOND_P) & SCT_EVn_CTRL_IOCOND_M)
#define SCT_EVn_CTRL_COMBMODE_P				12				/*!< How match and I/O condition are used and combined */
#define SCT_EVn_CTRL_COMBMODE_M				(0x3UL << SCT_EVn_CTRL_COMBMODE_P)
#define SCT_EVn_CTRL_COMBMODE_SET(value)	(((uint32_t)(value) << SCT_EVn_CTRL_COMBMODE_P) & SCT_EVn_CTRL_COMBMODE_M)
#define SCT_EVn_CTRL_STATELD					(1 << 14)			/*!< STATEV value is loaded/added to STATE */
#define SCT_EVn_CTRL_STATEV_P					15			/*!< This value is loaded into or added to the state selected by HEVENT */
#define SCT_EVn_CTRL_STATEV_M					(0x1FUL << SCT_EVn_CTRL_STATEV_P)
#define SCT_EVn_CTRL_STATEV_SET(value)	(((uint32_t)(value) << SCT_EVn_CTRL_STATEV_P) & SCT_EVn_CTRL_STATEV_M)
#define SCT_EVn_CTRL_MATCHMEM					(1 << 20)				/*!< If zero, a match is only active during cycle when match */
#define SCT_EVn_CTRL_DIRECTION_P			21			/*!< Direction qualifier for event generation */
#define SCT_EVn_CTRL_DIRECTION_M			(0x3UL << SCT_EVn_CTRL_DIRECTION_P)
#define SCT_EVn_CTRL_DIRECTION_SET(value)	(((uint32_t)(value) << SCT_EVn_CTRL_DIRECTION_P) & SCT_EVn_CTRL_DIRECTION_M)
...
typedef enum CHIP_SCT_EVENT {
	SCT_EVT0_BIT  = (1 << 0),	/*!< Event 0 */
	SCT_EVT1_BIT  = (1 << 1),	/*!< Event 1 */
	SCT_EVT2_BIT  = (1 << 2),	/*!< Event 2 */
	SCT_EVT3_BIT  = (1 << 3),	/*!< Event 3 */
	SCT_EVT4_BIT  = (1 << 4),	/*!< Event 4 */
	SCT_EVT5_BIT  = (1 << 5),	/*!< Event 5 */
	SCT_EVT6_BIT  = (1 << 6),	/*!< Event 6 */
	SCT_EVT7_BIT  = (1 << 7)	/*!< Event 7 */
} CHIP_SCT_EVENT_T;
 ...
 * @return	Nothing
 */
STATIC INLINE uint32_t Chip_SCT_GetCount(LPC_SCT_T *pSCT)
{
	return ( pSCT->COUNT_U );
}
