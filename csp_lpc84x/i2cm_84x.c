...
void Chip_I2CM_SetBusSpeed(LPC_I2C_T *pI2C, uint32_t busSpeed)
{
	...
	/* Get I2C clock rate */
	if ( pI2C == LPC_I2C0 ) {
		clk = Chip_Clock_GetFClockRate(5);
	}
	else if ( pI2C == LPC_I2C1 ) {
		clk = Chip_Clock_GetFClockRate(6);
	}
	/* Get I2C clock rate */
	if ( pI2C == LPC_I2C2 ) {
		clk = Chip_Clock_GetFClockRate(7);
	}
	else if ( pI2C == LPC_I2C3 ) {
		clk = Chip_Clock_GetFClockRate(8);
	}
	scl = clk / (Chip_I2C_GetClockDiv(pI2C) * busSpeed);
