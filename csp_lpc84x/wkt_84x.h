...
/** @defgroup WKT_84X CHIP: LPC8xx Self Wakeup Timer driver
...
#define WKT_CTRL_SEL_EXTCLK    ((uint32_t) (1 << 3))	/*!< Select ext. or int. clock source */
...
typedef enum  {
	...
	WKT_CLKSRC_WKTCLKIN = 2	/*!< External clock - runs at 1kHz */
} WKT_CLKSRC_T;
...
{
	if (pWKT->CTRL & WKT_CTRL_SEL_EXTCLK) {
		return (WKT_CLKSRC_T)(WKT_CLKSRC_WKTCLKIN);
	}
	else {
